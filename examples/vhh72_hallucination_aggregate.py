...
def load_all(results_dir: Path, seeds: list[int]):
    ...
    if not csv_paths:
        raise SystemExit(
            f"no {{policy}}_stopgrad{{0,1}}_seed{{{','.join(map(str, seeds))}}}.csv "
            f"files found in {results_dir}"
        )
    rows = []
...
def write_seed_variance_summary(rows, output_summary: Path):
    ...
    win_counts = {(policy, stop_grad): 0 for policy in POLICIES for stop_grad in STOP_GRAD_SETTINGS}
    n_seeds_compared = 0
    for seed, seed_rows in sorted(by_seed_at_max.items()):
        configs_present = {(r["policy"], r["stop_grad"]) for r in seed_rows}
        if configs_present != set(win_counts.keys()):
            print(f"  seed {seed}: skipping win-tally, missing configs "
                  f"({set(win_counts.keys()) - configs_present})", flush=True)
            continue
    ...
    print(f"\nwin-rate at edit_count={max_edit_count} (lowest total_loss per seed, "
          f"{n_seeds_compared} seeds with all 4 configs present):", flush=True)
...
def main():
    ...
    rows = load_all(args.results_dir, seeds)
    write_combined_csv(rows, args.output_csv)
    write_seed_variance_summary(rows, args.output_summary)

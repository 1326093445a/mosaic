...
RMSD against the real P17_JN1.pdb reference), answering directly: after
CDR mutation, is the binder still near the real binding region on the
target, or has it drifted? Mirrors
examples/vhh72_predict_hallucination_structures.py -- same division of
labor (cheap distogram-only path during search, real diffusion coordinate
path here, once per requested candidate, not thousands of times) and the
same eqx.filter_jit requirement at full complex size (see that script's
docstring for the exact OOM this avoids).
...
RMSD check is the primary validation available here, not a secondary
sanity check.
...
def main():
    ...
    with open(summary_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["tag", "seed", "edit_count",
                                                "real_binder_pose_rmsd", "mean_plddt", "cif"])
        writer.writeheader()
        writer.writerows(results)
    print(f"\nwrote summary: {summary_path}", flush=True)
    print("\nDone. Compare real_binder_pose_rmsd across rows against the edit_count=0 "
          "JN.1-baseline row's own value -- that baseline is the real reference point "
          "(OpenDDE's own prediction error on the true, unmutated sequence), not 0A. "
          "A design whose RMSD is close to the baseline's is not meaningfully drifted; "
          "one substantially higher is a real structural warning sign. Open the .cif "
          "files directly (e.g. PyMOL/ChimeraX) to inspect the pose visually.", flush=True)
